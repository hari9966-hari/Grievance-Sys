...
export const AdminOfficers = () => {
  ...
  const handleCreateOfficer = async (e) => {
    ...
    if (!formData.name || !formData.email || !formData.password || !formData.department) {
      showNotification(language === 'en' ? "Please fill all fields" : "அனைத்து புலங்களையும் நிரப்பவும்", 'warning');
      return;
    }
    ...
    try {
      ...
      fetchOfficers();
      showNotification('Officer account created successfully!', 'success');
    } catch (error) {
      showNotification(error.response?.data?.message || 'Error creating officer', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteOfficer = async (id) => {
    if (window.confirm(language === 'en' ? "Are you sure you want to delete this officer? This action cannot be undone." : "இந்த அதிகாரியை நீக்க விரும்புகிறீர்களா? இந்த நடவடிக்கையை மாற்ற முடியாது.")) {
      try {
        await adminAPI.deleteOfficer(id);
        fetchOfficers();
        showNotification('Officer deleted successfully', 'success');
      } catch (error) {
        showNotification(error.response?.data?.message || 'Error deleting officer', 'error');
      }
    }
  };
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-soft border border-neutral-100">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900">{t('admin.officersTitle')}</h1>
          <p className="text-sm text-neutral-500 mt-1">{t('admin.officersSubtitle')}</p>
        </div>
        <button 
        ...
        >
          ...
          <span className="hidden sm:inline">{t('admin.addOfficer')}</span>
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-neutral-200">
            <thead className="bg-neutral-50">
              <tr>
                <th className="px-6 py-4 text-left font-semibold text-neutral-600 tracking-wider font-semibold text-neutral-600 tracking-wider">
                  {t('admin.name')}
                </th>
                <th className="px-6 py-4 text-left font-semibold text-neutral-600 tracking-wider hidden sm:table-cell">
                  {t('admin.email')}
                </th>
                <th className="px-6 py-4 text-left font-semibold text-neutral-600 tracking-wider hidden md:table-cell">
                  {t('admin.department')}
                </th>
              ...
              {officers.length > 0 ? (
                officers.map((officer) => (
                  <tr key={officer._id} className="hover:bg-neutral-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-neutral-600 hidden md:table-cell">
                      {officer.department ? (
                        <span className="bg-neutral-100 px-2 py-1 rounded-md text-neutral-700 font-medium text-xs border border-neutral-200">
                          {language === 'en' ? officer.department : (translations.ta.categories?.[officer.department] || officer.department)}
                        </span>
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-2 items-start">
                        <span className="bg-primary-50 text-primary-700 px-2 py-0.5 rounded-md font-medium text-xs border border-primary-200/50">
                          {language === 'en' ? 'Level' : 'நிலை'} {officer.authorityLevel}
                        </span>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${
                          ...
                          {officer.isActive ? t('admin.active') : t('admin.inactive')}
                        </span>
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-neutral-500">
                    {language === 'en' ? 'No officers have been added yet.' : 'அதிகாரிகள் இன்னும் சேர்க்கப்படவில்லை.'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)}>
        <div className="flex justify-between items-center border-b border-neutral-100 pb-4 mb-4">
          <h3 className="text-lg font-semibold text-neutral-900">{t('admin.addModalTitle')}</h3>
        ...
        <form onSubmit={handleCreateOfficer} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">{t('admin.name')}</label>
            <input 
              type="text" 
              value={formData.name}
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">{t('admin.email')}</label>
            <input 
              type="email" 
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">{t('admin.department')}</label>
            <select 
              ...
              required
            >
              <option value="">{language === 'en' ? 'Select a Department' : 'துறையைத் தேர்ந்தெடுக்கவும்'}</option>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-neutral-700 mb-1">{t('admin.authorityLevel')}</label>
              <select 
                value={formData.authorityLevel}
                onChange={(e) => setFormData({...formData, authorityLevel: parseInt(e.target.value)})}
                className="w-full rounded-xl border-neutral-300 py-2.5 px-3 text-neutral-900 focus:ring-primary-500 focus:border-primary-500 sm:text-sm bg-neutral-50 hover:bg-white transition-colors"
              >
                <option value={1}>{language === 'en' ? 'Level 1 (Standard)' : 'நிலை 1 (தரமானது)'}</option>
                <option value={2}>{language === 'en' ? 'Level 2 (Supervisor)' : 'நிலை 2 (கண்காணிப்பாளர்)'}</option>
                <option value={3}>{language === 'en' ? 'Level 3 (Director)' : 'நிலை 3 (இயக்குனர்)'}</option>
              </select>
            </div>
            <div>
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-neutral-100">
            ...
            >
              {submitting ? (language === 'en' ? 'Creating...' : 'உருவாக்குகிறது...') : (language === 'en' ? 'Create Officer' : 'அதிகாரியை உருவாக்கு')}
            </button>
          </div>
        </form>
      </Modal>
